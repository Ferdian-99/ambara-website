...
export function PortfolioDetail() {
  ...
  return (
    <PageShell eyebrow={project.category} title={project.title} intro={project.concept}>
      <section className="section-wrap"><div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]"><VisualBlock tone={project.tone} tall imageSrc={project.imageSrc} imageAlt={project.imageAlt} /><div className="project-meta"><div><span>Tipe</span><p>{project.type}</p></div><div><span>Tahun</span><p>{project.year}</p></div><div><span>Lokasi</span><p>{project.location}</p></div></div></div></section>
      ...
      <section className="section-wrap"><div className="grid gap-6 md:grid-cols-3">{[0, 1, 2].map((item) => <Reveal key={item}><VisualBlock tone={project.tone} imageSrc={project.imageSrc} imageAlt={`${project.imageAlt} - galeri ${item + 1}`} /></Reveal>)}</div></section>
